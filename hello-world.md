...
a string if it is not one already[^hello-world-1] (like `operator<<()` in C++).
...
it "bar" from `main`)[^hello-world-2].
...
[^hello-world-1]: This is a programmer specified conversion which uses the `Display` trait, which
...
[^hello-world-2]: We don't actually use that argument in `foo`. Usually,
